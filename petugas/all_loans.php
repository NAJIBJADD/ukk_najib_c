...
$loans = [];
if ($showLoans) {
    // Query untuk menampilkan loan yang belum diajukan hapus (pending) atau sudah ditolak
    $sqlLoans = "SELECT l.*, u.nama_lengkap AS siswa, u.nis, i.nama_item 
                 FROM loans l 
                 JOIN users u ON l.id_siswa = u.id 
                 JOIN items i ON l.id_item = i.id 
                 LEFT JOIN delete_requests dr ON l.id = dr.loan_id AND dr.status = 'pending'
                 WHERE dr.id IS NULL";
    $params = [];
    if ($filter == 'telat') {
        $sqlLoans .= " AND l.status = 'dipinjam' AND l.batas_waktu < NOW()";
    } elseif ($filter != 'all') {
        $sqlLoans .= " AND l.status = ?";
        $params[] = $filter;
    }
    $sqlLoans .= " ORDER BY l.tgl_pinjam DESC";
...
if ($filter == 'all' || $filter == 'pending') {
    ...
    $pendingRequests = $pdo->query($sqlPending)->fetchAll(PDO::FETCH_ASSOC);
}
...
if ($filter == 'ditolak' || $filter == 'all') {
    ...
    $rejectedRequests = $pdo->query($sqlRejected)->fetchAll(PDO::FETCH_ASSOC);
}
?>
...
<style>
    *, *::before, *::after {
    ...
    body, .card, .card-body, .card-header, .table, .table tbody tr, .table tbody td, .table thead th {
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <ul class="nav nav-pills mb-4">
                ...
                <li class="nav-item"><a class="nav-link <?= $filter == 'kembali' ? 'active' : '' ?>" href="?filter=kembali">Kembali</a></li>
                ...
                <div class="table-responsive mb-4">
                    <table class="table table-bordered table-striped">
                        <thead class="table-dark">
                            <tr><th>ID Siswa</th><th>Siswa</th><th>NIS</th><th>Barang</th><th>Tgl Request</th><th>Tanggal Kembali Diminta</th><th>Status</th><th>Aksi</th></tr>
                        </thead>
                        <tbody>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <?php foreach ($loans as $loan): 
                                ...
                                $conditionText = ($loan['status'] == 'rusak') ? 'Rusak (50.000)' : (($loan['status'] == 'hilang') ? 'Hilang (200.000)' : '-');
                            ?>
                                <tr class="<?= $is_late ? 'table-danger' : '' ?>">
                                    ...
                                    <td><?= format_tanggal_indonesia($loan['tgl_pinjam']) ?></td>
                                    <td><?= format_tanggal_indonesia($loan['batas_waktu']) ?></td>
                                    <td><?= $loan['tgl_kembali'] ? format_tanggal_indonesia($loan['tgl_kembali']) : '-' ?></td>
                                    <td><span class="badge bg-<?= $badge_class ?>" style="font-weight: bold; color: #000;"><?= strtoupper($status_display) ?></span></td>
                                    <td>
                                        <?php if ($lateFine > 0) {
                                            echo 'Rp ' . number_format($lateFine,0,',','.') . ' (' . ($lateFine/1000) . ' hari)';
                                        } else {
                                            echo '-';
                                        } ?>
                                    </td>
                                            ...
                                            <div class="modal fade" id="deleteModal<?= $loan['id'] ?>" tabindex="-1">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-danger text-white"><h5 class="modal-title">Ajukan Penghapusan Data</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                                                        <form method="POST" action="request_delete.php">
                                                            <input type="hidden" name="loan_id" value="<?= $loan['id'] ?>">
                                                            <div class="modal-body"><div class="mb-3"><label>Alasan penghapusan</label><textarea name="alasan" class="form-control" rows="3" required placeholder="Contoh: Data duplikat, salah input"></textarea></div></div>
                                                            <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-danger">Kirim Permintaan</button></div>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
            ...
            <?php if (($filter == 'all' || $filter == 'ditolak') && !empty($rejectedRequests)): ?>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <thead class="table-dark">
                            <tr><th>ID Siswa</th><th>Siswa</th><th>NIS</th><th>Barang</th><th>Tgl Request</th><th>Tanggal Kembali Diminta</th><th>Status</th><th>Catatan</th><tr>
                        </thead>
                        <tbody>
